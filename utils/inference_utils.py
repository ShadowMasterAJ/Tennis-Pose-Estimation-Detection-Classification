...
def draw_keypoints_and_skeleton(img, keypoints):
    ...
    scale_factor = 5  # Adjust this value to increase or decrease the scale
    ...
    for i, (start, end) in enumerate(skeleton):
        if all(kp > 0 for kp in keypoints[0][start-1]) and all(kp > 0 for kp in keypoints[0][end-1]):
            cv2.line(black_frame, scaled_keypoints[start-1], scaled_keypoints[end-1], joint_colors[i % len(joint_colors)], 2)
    ...
    for i, (start_point, end_point) in enumerate(skeleton):
        start_idx, end_idx = start_point - 1, end_point
    ...
    for i, (start, end) in enumerate(skeleton):
        if all(kp > 0 for kp in keypoints[0][start-1]) and all(kp > 0 for kp in keypoints[0][end-1]):
            cv2.line(img, keypoints[0][start-1], keypoints[0][end-1], joint_colors[i % len(joint_colors)], 2)
